...
| **`[`** | 窗口聚焦时 | 后退到上一个查询 |
| **`]`** | 窗口聚焦时 | 前进到下一个查询 |
| **`↓`** | 搜索框聚焦时 | 展开下一个结果区域（建议可见时导航建议） |
| **`↑`** | 搜索框聚焦时 | 收缩上一个结果区域（建议可见时导航建议） |
| **Esc** | 窗口聚焦时 | 隐藏词典窗口（焦点退回原应用） |
| **Ctrl+A/E/B/F** | 搜索框聚焦时 | 移动光标，进入编辑模式（追加而非覆盖） |
| **Tab** | 建议列表可见时 | 采纳首条建议 |
| **`↓`/`↑`** | 建议列表可见时 | 选择建议，Enter 采纳 |
...
输入时自动从查询历史中前缀匹配，按长度升序+字典序排列，首项默认高亮：
- **Tab** 立即采纳首条建议
- **`↓`/`↑`** 浏览列表，**Enter** 采纳选中项
- **Esc** 首次隐藏建议，再次隐藏窗口
...
- **搜索建议** — 输入时自动从查询历史中前缀匹配，Tab 采纳、方向键浏览。
- **考试标签** — 释义上方显示该词所属考试（高中、四级、六级、考研、雅思、GRE），圆角标签一目了然。
- **查询次数** — 每个单词累计查询次数以标签形式展示，掌握高频词汇。
