...
| 按键 | 场景 | 功能 |
|---|---|---|
| **右 Command** | 任意位置 | 唤起/隐藏词典窗口（隐藏时焦点退回原应用） |
| **右 Option** | 窗口可见时 | 切换焦点：搜索框 ↔ 原应用 |
| **Enter** | 搜索框聚焦时 | 查询当前输入 |
...
| **`↓`** | 搜索框聚焦时 | 展开下一个结果区域 |
| **`↑`** | 搜索框聚焦时 | 收缩上一个结果区域 |
| **Esc** | 窗口聚焦时 | 隐藏词典窗口（焦点退回原应用） |
| **Ctrl+A/E/B/F** | 搜索框聚焦时 | 移动光标，进入编辑模式（追加而非覆盖） |

### 智能编辑

查询完一个单词后，直接键入新字符会**覆盖**原词——方便快速查新词。先按 `Ctrl+A`、`Ctrl+E`、`Ctrl+B` 或 `Ctrl+F` 则进入**编辑模式**，之后键入会在原词上追加而非覆盖。

## 核心功能

- **全局快捷键** — 在任意 App 内按右 Command 即可唤起或隐藏窗口，隐藏时焦点自动退回原应用原光标位置。
- **焦点切换** — 右 Option 可在搜索框与原应用之间来回切换焦点，无需隐藏窗口即可跳回阅读。
